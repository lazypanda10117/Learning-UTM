...
class UniversalTuringMachine(abc.ABC):
  ...
  def sample_program_with_markov_process(
      self,
      length: int,
      markov_data: dict[str, dict[str, float]],
      rng: np.random.Generator,
  ) -> str:
    """Returns a program of a given length, with tokens sampled from a Markov process."""
    program = ''
    for _ in range(length):
      suffix = program[-2:]
      if suffix not in markov_data:
        raise ValueError(f"Suffix {suffix} not in markov process")
      distribution = markov_data[suffix]
      token = rng.choice(list(distribution.keys()), p=list(distribution.values()))
      program += token
    return program
